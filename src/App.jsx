  ...
  return (
    <div style={{ padding: '18px 20px 40px', maxWidth: 560 }}>
      ...
      <div className="xv-card" style={{ padding: 16, marginBottom: 14 }}>
        <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 4 }}>This app's current data</div>
        {dataSource.mode === 'live' ? (
          <div style={{ fontSize: 12.5, color: 'var(--text-dim)', lineHeight: 1.5 }}>
            <span className="xv-chip" style={{ marginRight: 6 }}>Live</span>
            Showing data from your backend, last checked {dataSource.checkedAt ? new Date(dataSource.checkedAt).toLocaleString() : 'just now'}.
          </div>
        ) : (
          <div style={{ fontSize: 12.5, color: 'var(--text-dim)', lineHeight: 1.5 }}>
            <span className="xv-chip" style={{ marginRight: 6 }}>Bundled sample data</span>
            {DEVICE_SOURCE === 'api'
              ? `Couldn't reach the live backend (${dataSource.error || 'no response'}) at ${API_BASE_URL} — showing the bundled catalogue instead so the app still works.`
              : `This build is set to always use bundled sample data (DEVICE_SOURCE='bundled' in src/lib/api.js) rather than a live backend.`}
          </div>
        )}
      </div>

  ...
  return (
    <div className="xenvia" data-theme={theme} style={{ display: 'flex' }}>
      ...
      <main style={{ flex: 1, minWidth: 0, paddingBottom: 76 }}>
        ...
        {view === 'settings' && (
          <SettingsView theme={theme} setTheme={setTheme} deviceCount={MASTER_DEVICES.length} brandCount={brandList.length} />
        )}
      </main>
  ...
  return (
    <div style={{ padding: '18px 20px 40px', maxWidth: 880, margin: '0 auto' }}>
      ...
      <div className="xv-card" style={{ padding: 18, marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text-dim)', marginTop: 10, lineHeight: 1.5 }}>{gamingRationale(device)}</div>
        {specMode === 'detailed' && (
        ...
        return (
          <>
            ...
            <SpecSection title="Battery & charging" icon={BatteryCharging} rows={[
              ['Capacity', `${device.battery} mAh`], ['Type', batt.type], ['Wired charging', `${device.charging}W`], ['Fast-charge tech', batt.fastChargeTech],
...
function CompareView({ devices, getSelectedVariant, onSelectVariant, onRemove, onOpen }) {
  ...
  return (
    <div style={{ padding: '18px 20px 40px' }}>
      ...
      {/* Desktop table */}
      <div className="hidden md:block xv-card" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: `160px repeat(${devices.length}, 1fr)`, borderBottom: '1px solid var(--border)' }}>
          <div />
          {devices.map((d, i) => (
            <div key={d.id} style={{ padding: 14, borderLeft: '1px solid var(--border)' }}>
              <button onClick={() => onOpen(d.id)} style={{ background: 'none', border: 'none', textAlign: 'left', padding: 0, width: '100%' }}>
                <div style={{ fontSize: 11, color: 'var(--text-dim)' }}>{d.brand}</div>
                <div style={{ fontWeight: 600, fontSize: 13.5 }}>{d.name}</div>
              </button>
              <div style={{ display: 'flex', gap: 5, marginTop: 8, flexWrap: 'wrap' }}>
                {d.variants.map((v) => (
                  <button key={v.id} className={`xv-chip ${v.id === variants[i].id ? 'active' : ''}`} style={{ fontSize: 10.5, padding: '3px 8px' }} onClick={() => onSelectVariant(d, v.id)}>{v.ram}/{v.rom}</button>
                ))}
              </div>
              <button className="xv-btn xv-btn-icon" style={{ marginTop: 8 }} onClick={() => onRemove(d.id)} aria-label="Remove"><Trash2 size={13} /></button>
            </div>
          ))}
        </div>
        {COMPARE_ROWS.map((row) => {
          ...
          return (
            <div key={row.label} style={{ display: 'grid', gridTemplateColumns: `160px repeat(${devices.length}, 1fr)`, borderBottom: '1px solid var(--border)' }}>
              <div style={{ padding: 14, color: 'var(--text-dim)', fontSize: 12.5 }}>{row.label}</div>
              {devices.map((d, i) => {
                const raw = row.get(d, variants[i]);
                return (
                  <div key={d.id} style={{ padding: 14, borderLeft: '1px solid var(--border)', fontSize: 12.5 }} className="xv-mono">
                    <span style={i === best ? { color: 'var(--pos)', fontWeight: 700 } : {}}>{row.fmt(raw, d)}</span>
                    {i === best && <Check size={11} style={{ marginLeft: 4, color: 'var(--pos)' }} />}
                  </div>
                );
              })}
            </div>
      ...
      <div className="md:hidden" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {devices.map((d, i) => (
          <div key={d.id} className="xv-card" style={{ padding: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <div style={{ fontSize: 11, color: 'var(--text-dim)' }}>{d.brand}</div>
                <div style={{ fontWeight: 600 }}>{d.name}</div>
              </div>
              <button className="xv-btn xv-btn-icon" onClick={() => onRemove(d.id)}><Trash2 size={13} /></button>
            </div>
            {COMPARE_ROWS.map((row) => {
              const best = bestIndex(row);
              const raw = row.get(d, variants[i]);
              return (
                <div className="xv-spec-row" key={row.label}>
                  <span className="xv-spec-label">{row.label}</span>
                  <span className="xv-spec-value" style={i === best ? { color: 'var(--pos)', fontWeight: 700 } : {}}>{row.fmt(raw, d)}</span>
                </div>
              );
            })}
          </div>
...
// Settings
// ---------------------------------------------------------------------------
function SettingsView({ theme, setTheme, deviceCount, brandCount }) {
  return (
    <div style={{ padding: '18px 20px 40px', maxWidth: 560 }}>
      ...
      <div className="xv-card" style={{ padding: 16, marginBottom: 14 }}>
        ...
        <div style={{ fontSize: 12.5, color: 'var(--text-dim)', marginTop: 6, lineHeight: 1.5 }}>
          AI-powered chat, natural-language comparisons, and smart collections are architected but intentionally turned off in this version — the encyclopedia works fully without them. Coming soon.
        </div>
        <div style={{ marginTop: 10 }}><span className="xv-chip">Disabled</span></div>
      </div>

      <div className="xv-card" style={{ padding: 16, marginBottom: 14 }}>
        <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 4 }}>Data sources & sync</div>
        <div style={{ fontSize: 12.5, color: 'var(--text-dim)', marginBottom: 10, lineHeight: 1.5 }}>
          Designed to refresh daily at 03:00 from official manufacturer sites and PriceOye.pk as the top-priority sources, falling back to GSM Arena and WhatMobile when needed — specs, pricing, and product imagery all follow this same priority order. This is architected and ready to connect, but a live scrape can't run inside a browser-based artifact — it needs a small always-on service outside this environment. Until that's connected, the catalogue uses bundled sample data and stylized device art instead of hotlinked photos, so nothing ever shows as a broken image.
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 10 }}>
          {DATA_SOURCES.map((s) => (
            <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12.5 }}>
              <span>{s.name}</span>
              <span className="xv-chip" style={{ padding: '2px 8px', fontSize: 10.5 }}>{s.role}</span>
            </div>
          ))}
        </div>
        <div style={{ fontSize: 11.5, color: 'var(--text-dim)' }} className="xv-mono">{DataSyncService.lastSyncedLabel()} · window: last {CATALOG_WINDOW_YEARS} years (older devices still browsable via filters)</div>
      </div>

      <div className="xv-card" style={{ padding: 16 }}>
        <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 8 }}>About this catalogue</div>
        <div style={{ fontSize: 12.5, color: 'var(--text-dim)', lineHeight: 1.6 }}>
          {deviceCount} devices across {brandCount} brands. Pricing is illustrative and informational, resolved with priceoye.pk given top priority and WhatMobile / GSM Arena as secondary reference sources — Xenvia is a research and discovery tool, not a store. Your wishlist and collections are saved to this browser only.
        </div>
      </div>
    </div>
  );
